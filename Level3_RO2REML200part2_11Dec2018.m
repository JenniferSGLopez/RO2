...
data = importdata('Level3_RO2SufficientSubSampleDetermination_12Dec2018.txt',' ',1);
...
%D=D((2*ceil(size(D,1)/8)+1):((3*ceil(size(D,1)/8))),:);
%fprintf(' %d cases to run.\n',size(D,1));
%
% % Run fourth part of sample set
%D=D((3*ceil(size(D,1)/8)+1):((4*ceil(size(D,1)/8))),:);
%fprintf(' %d cases to run.\n',size(D,1));
%
% % Run fifth part of sample set
%D=D((4*ceil(size(D,1)/8)+1):((5*ceil(size(D,1)/8))-1),:);
%fprintf(' %d cases to run.\n',size(D,1));
%
% % Run sixth part of sample set
%D=D((5*ceil(size(D,1)/8)):((6*ceil(size(D,1)/8)-1)),:);
%fprintf(' %d cases to run.\n',size(D,1));
%
% % Run seventh part of sample set
%D=D(((6*ceil(size(D,1)/8))):((7*ceil(size(D,1)/8)-2)),:);
%fprintf(' %d cases to run.\n',size(D,1));
%
...
csvwrite('Level3_RO2_REML200part2Block_11Dec2018.csv', MC_history);
